...
contract Presale is Ownable, Pausable, ReentrancyGuard {
    ...
    event NoobPurchased(address user, uint32 amount, uint32 buyPrice);
    ...
    function AmountPurchased() external view returns (uint32) {
        return Purchased[msg.sender];
    }

    function GetBuyPrice(uint32 amount) external view returns (uint32) {
        return BuyPrice;
    }

    function Buy(bytes32[] calldata proof, uint32 amount, uint32 maxAmount) payable external nonReentrant {
        uint32 finalPrice = BuyPrice * amount;
    ...
    function _recordPurchase(uint32 amount, uint32 maxAmount) private {
    ...
    function _verifyProof(bytes32[] memory proof, address addr, bytes32 root, uint32 amount) private pure {   
    ...
    function UpdateBuyPrice(uint32 price) external onlyOwner {
        BuyPrice = price;
    }
