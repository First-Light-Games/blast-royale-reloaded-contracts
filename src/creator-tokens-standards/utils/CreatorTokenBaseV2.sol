...
import "@openzeppelin-v4/contracts/interfaces/IERC165.sol";
...
abstract contract CreatorTokenBaseV2 is OwnablePermissions, TransferValidation, ICreatorToken {
    ...
    function setToDefaultSecurityPolicy() public virtual {
        ...
        ICreatorTokenTransferValidatorV2(DEFAULT_TRANSFER_VALIDATOR).setTransferSecurityLevelOfCollection(
            address(this), DEFAULT_TRANSFER_SECURITY_LEVEL
        );

        ICreatorTokenTransferValidatorV2(DEFAULT_TRANSFER_VALIDATOR).applyListToCollection(
            address(this), DEFAULT_LIST_ID
        );
    }
     ...
     */
    function setToCustomValidatorAndSecurityPolicy(address validator, TransferSecurityLevels level, uint120 listId)
        public
    {
        _requireCallerIsContractOwner();
     ...
     * @param listId The id of the list to set for the collection.
     */
    function setToCustomSecurityPolicy(TransferSecurityLevels level, uint120 listId) public {
        _requireCallerIsContractOwner();
    ...
    function setTransferValidator(address transferValidator_) public {
        ...
        transferValidatorReference =
            TransferValidatorReference({isInitialized: true, version: version, transferValidator: transferValidator_});
    }
    ...
    function setToCustomValidatorAndSecurityPolicy(
        ...
        if (validator != address(0)) {
            ...
            ICreatorTokenTransferValidator(validator).setOperatorWhitelistOfCollection(
                address(this), operatorWhitelistId
            );
