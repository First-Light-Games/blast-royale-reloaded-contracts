...
contract ERC721M is IERC721M, ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    function setStages(MintStageInfo[] calldata newStages) external onlyOwner {
        ...
        for (uint256 i = 0; i < newStages.length; i++) {
            if (i >= 1) {
                if (newStages[i].startTimeUnixSeconds < newStages[i - 1].endTimeUnixSeconds + timestampExpirySeconds) {
    ...
    function updateStage(
        ...
        if (index >= 1) {
            if (startTimeUnixSeconds < _mintStages[index - 1].endTimeUnixSeconds + getTimestampExpirySeconds()) {
                revert InsufficientStageTimeGap();
            }
        }
     ...
     */
    function _mintInternal(uint32 qty, address to, bytes32[] calldata proof, uint64 timestamp, bytes calldata signature)
        internal
        canMint
        hasSupply(qty)
    {
        ...
        if (stage.merkleRoot != 0) {
            if (MerkleProof.processProof(proof, keccak256(abi.encodePacked(to))) != stage.merkleRoot) {
                revert InvalidProof();
            }
        }
