...
abstract contract AdventureBase is AdventureWhitelist, IAdventurous {
    ...
    function _exitQuest(uint256 tokenId, address adventure, uint256 questId) internal {
        (bool participatingInQuest, uint256 startTimestamp, uint256 index) =
        ...
        if (index != lastArrayIndex) {
            ...
            activeQuestLookup[tokenId][adventure][activeQuestList[tokenId][adventure][lastArrayIndex]].arrayIndex =
                uint32(index);
        }
...
abstract contract AdventureERC721 is AdventureBase, ERC721OpenZeppelin {
    ...
    function supportsInterface(bytes4 interfaceId) public view virtual override(ERC721, IERC165) returns (bool) {
        return interfaceId == type(IAdventurous).interfaceId || super.supportsInterface(interfaceId);
    }
